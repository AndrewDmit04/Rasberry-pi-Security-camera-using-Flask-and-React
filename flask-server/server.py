...
########setup###########
SERVER_IP = 'YOUR IP'                #run ifconfig for Mac and Linux or ipconfig for Windows to find out ip of Rasberry and put it in here 
MAX_FACES_IN_FILE = 200              #maximum amount of faces insideof the faces folder will delete the oldest faces 
MAX_RECORDINGS_IN_FILE = 100         #maximum amount of recordings in the Recordings folder will delete the oldest videos 
MAX_RECENT_FACES = 10                #maximum amount of recent faces displayed on the screen
...
def add_faces(frame,faces):                                                                 #add faces to the recent faces buffer
    num_of_faces = 1                                                                        #num of faces in case there is more than one face in a frame
    for(x,y,w,h) in faces:                                                                  #get the coordinates of the faces in the frame
        ...
        num_of_faces += 1                                                                   #add one to the num of faces if there are more faces then the file will have a different name
...
def getFileName(camera):                                                                    #function to get the file name for the video being recorded
    ...
    ret,frame = camera.read()                                                               #get the current frame for the thumbnail of the video
...
def generate_video_feed():
    ...
    while True:
        ...
        if time.time() - startTimeRecord > 360:                                             #every 6 minutes start a new recording
        ...
        try:
            ...
            pass                                                                            #if doesn't work pass
   
    cap.release()                                                                           #if a loop exists release the camera
