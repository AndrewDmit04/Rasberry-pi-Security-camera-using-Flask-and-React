...
def sort_times(dates):
    formated = [file.split("_")[0] for file in dates] #get the all the dates of the images as a list
...
def delete_old_videos():
    ...
    if len(video_files) > MAX_RECORDINGS_IN_FILE:
        video_files = [file[:-4] for file in video_files]                                       #get rid of the extention on at the back of the name
...
def delete_old_faces():                       #function will delete all old faces in the file
    ...
    if len(face_photos) > max_faces:          #if the length is more than it should be
        sorted_faces = sort_times(face_photos) #sort the faces based on the date returns oldest last
...
def add_faces(frame,faces):         #add faces to the recent faces buffer
    num_of_faces = 1                #num of faces incase theres more than one face in a frame
    for(x,y,w,h) in faces:          #get the cordinates of the faces in the frame
        ...
        num_of_faces += 1   #add one to num of faces if more faces then the file will have a diffrent name
    while len(face_buffer) > MAX_RECENT_FACES:  #if the buffer length is more than it should be keep poping the faces until its right
...
def generate_video_feed():
    ...
    out = cv.VideoWriter(getFileName(cap), fourcc, 20.0, (640, 480))   #set a out to write the frames to
...
def serve_video(filename):
    ...
    return send_file(video_path,as_attachment=True, mimetype='video/x-msvideo')             #return the video as an attachment to dowload it for the user
...
if __name__ == '__main__':
    video_thread = threading.Thread(target=generate_video_feed) #start a thread that constantly records even if the page isnt opened
